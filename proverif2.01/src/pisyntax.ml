...
let check_event state ((f,e),n) =
	...
	match tl0 with
	  [PGFunApp(id, tl),_] ->
	    if !Param.key_compromise == 0 then
	      QSEvent(None,[], None,FunApp((find_event_arity state id (List.length tl)),
				      List.map (check_query_term state) tl))
	    else
	      QSEvent(None,[], None,FunApp((find_event_arity state id (1+List.length tl)),
				    (Terms.new_var_def_term Param.any_type)::
				    (List.map (check_query_term state) tl)))
  ...
  | PGSimpleFact(("evinj",e'),tl0) ->
	...
	match tl0 with
	  [PGFunApp(id, tl),_] ->
	    if !Param.key_compromise == 0 then
	      QSEvent(Some (Param.fresh_injective_index ()),[], None,FunApp((find_event_arity state id (List.length tl)),
				   List.map (check_query_term state) tl))
	    else
	      QSEvent(Some (Param.fresh_injective_index ()),[], None,FunApp((find_event_arity state id (1+List.length tl)),
				   (Terms.new_var_def_term Param.any_type)::
				   (List.map (check_query_term state) tl)))
...
let check_real_query_top state = function
    PBefore(ev, hypll) ->
      ...
      let ev'' =
	match ev' with
	  QNeq _ | QEq _ | QGeq _ | QIsNat _ -> user_error "Inequalities, disequalities or equalities cannot occur before ==> in queries"
	| QFact _ -> ev'
	| QSEvent _ when !Param.key_compromise == 0 -> ev'
	| QSEvent(inj,[], None,FunApp(f, sid::l)) ->
	    QSEvent(inj,[], None, FunApp(f, non_compromised_session::l))
	| QSEvent _
        | QSEvent2 _->
...
let transl_query events pred_env glob_table q pi_state =
  ...
  CorrespQuery(List.map Reduction_helper.check_inj_coherent q'',Param.dummy_solve_status)
