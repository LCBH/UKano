...
let rec copy_process add_in_glob_table = function
  ...
  | Input(t, pat, p, occ) ->
	...
	Input(Terms.copy_term3 t, pat',
	      copy_process add_in_glob_table p, new_occurrence ~precise:occ.precise ()))
  | Output(tc,t,p, occ) ->
...
let copy_process_desc add_in_glob_table p = 
  { p with proc = copy_process add_in_glob_table p.proc; display_num = None }
...
let prepare_process pi_state =
  ...
  let pi_process_query' =
	...
	SingleProcess(p,ql) ->
	  let p' = copy_process_desc add_in_glob_table p in
	  SingleProcess(p',ql)
      | SingleProcessSingleQuery(p,q) ->
	  let p' = copy_process_desc add_in_glob_table p in
	  SingleProcessSingleQuery(p',q)
      | Equivalence(p1,p2) ->
	  let p1' = copy_process_desc add_in_glob_table p1 in
	  let p2' = copy_process_desc add_in_glob_table p2 in
	  Equivalence(p1',p2')
	    )
  in
  { pi_state with
...
let rec move_new_lets accu = function
  ...
  | LetFilter(vl,(Pred(_,l) as fact),p,q,occ) ->
     if q <> Nil then
       put_new_lets accu (LetFilter(vl,fact, move_new_lets [] p, move_new_lets [] q, occ))
     else
	let (l1,l2) = partition_new_lets l accu in
        put_new_lets l1 (LetFilter(vl,fact, move_new_lets l2 p, Nil, occ))
  | Event(t1,env_args,p,occ) ->
     let (l1,l2) = partition_new_lets [t1] accu in
     put_new_lets l1 (Event(t1,env_args,move_new_lets l2 p,occ))
  | Test(t,p1,p2,occ) ->
      if p2 <> Nil then
