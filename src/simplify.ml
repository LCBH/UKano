...
let rec verify_unification proc acc_unify = 
  ...
  then canonical_process proc acc_unify
    ...
    begin 
      if proc <> Nil
      then print_string "Warning: Some part of the protocol will never be executed\n";
...
and canonical_process process acc_unify = match process with
  ...
  | Event (_,_,_) -> internal_error "[simplify.ml >> canonical_process] Event should not be in the process"
